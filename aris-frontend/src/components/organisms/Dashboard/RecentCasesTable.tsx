...
const statusConfig: Record<string, { bg: string; text: string; dot: string; label: string }> = {
  OPEN: { bg: "bg-amber-50", text: "text-amber-800", dot: "bg-amber-400", label: "Open" },
  IN_PROGRESS: { bg: "bg-blue-50", text: "text-blue-800", dot: "bg-blue-400", label: "In Progress" },
  COMPLETED: { bg: "bg-emerald-50", text: "text-emerald-800", dot: "bg-emerald-400", label: "Completed" },
};
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
        <div>
          <h3 className="font-bold text-slate-800">{t("dashboard.recentCases")}</h3>
          <p className="mt-0.5 text-xs text-slate-400">Last 30 days activity</p>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50/70">
              {["Case ID", "Incident", "Institution", "Date", "Stage", "Status"].map((heading) => (
                <th key={heading} className="whitespace-nowrap px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">
                  {heading}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-slate-50">
            {isLoading ? (
              [...Array(5)].map((_, index) => (
                <tr key={index}>
                  <td colSpan={6} className="px-4 py-4"><div className="h-4 animate-pulse rounded bg-slate-100" /></td>
                </tr>
              ))
            ) : cases.length ? (
              cases.map((caseItem) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className="inline-flex whitespace-nowrap rounded-md bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-700">
